...
| `/models`                 | Model catalog, factor inspector, a structured admin model builder (field-by-field factors + live JSON preview, Advanced-JSON fallback), **edit**, and **archive / delete** of an existing model | `/templates`, `/scores/factors`, `/scores/{kinds,types}`, `/admin/models`, `/admin/models/{definition,update,archive,delete}` |
...
| `/financials`             | Financial-fact stewardship: **record a source's values**, **edit a fact's value inline**, and resolve source conflicts (`data:write`)                                                           | `/financials`, `/financials/{conflicts,concepts,sources}`, `POST /financials/{observations,canonical,value}`                  |
